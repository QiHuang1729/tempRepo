 ...
 *
 *	@author	Scott DeRuiter and David Greenstein and Qi Huang
 *	@version	1.0
 *	@since	10/12/2023
 */ 
public class Wordle
{ 
	...
	{
		// I can either store each word in the file into an array and 
		// check those, or I can open the scanner and check each word in
		// the array to see if it is equal to the testWord
		// if it isn't, the I have to look through the scanner again
		// to find a random word. How will I find a random word?
		
		String result = "SMART";
		String possibleWord = new String("");
		boolean valid = false;
		Scanner goalWords = FileUtils.openToRead(inFileName);
		
		if (testWord.length() == 5) {
			while (goalWords.hasNext() && !valid) {
				possibleWord = goalWords.nextLine();
				if (possibleWord.equalsIgnoreCase(testWord)) {
